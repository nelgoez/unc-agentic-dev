...
function main(): void {
    ...
    const visibilityPhantomCount = data.apiFindings.filter(
        f => f.type === 'api-visibility-phantom' || f.type === 'api-visibility-restricted',
    ).length;
    console.log(
    ...
    if (visibilityPhantomCount > 0) {
        console.log(
            `   Visibility phantom findings: ${visibilityPhantomCount} (mapped via severity:critical)`,
        );
    }
    console.log(
