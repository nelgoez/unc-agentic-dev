# Plan: Phase 2 — Captioned Playwright Steps for Non-IT Audience

## Global Constraints

- All existing tests must continue to pass
- The CI pipeline must not break (audit-ci.yml stays unchanged)
- Spanish-language captions for all user-facing output
- Screenshots must be embedded in the HTML report (already done)
- Use `test.step('caption', async () => { ... })` for each step
...
test.step('3. Captura de pantallas por sección (administrador)', ...)
...
### Task 2: (Future) — captions in generate-audit-report.ts

For Phase 3: Show step captions in the HTML report alongside screenshots.
