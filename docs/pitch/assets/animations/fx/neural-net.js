(function () {
  ...
  window.HPX['neural-net'] = function (el) {
    ...
    const ac = U.accent(el, '#7c5cff'); const ac2 = U.accent2(el, '#22d3ee');
    const layers = [4, 6, 6, 3];
    let nodes = []; let edges = []; let pulses = [];
    const layout = () => {
      nodes = [];
      const pad = 40;
      const cw = k.w - pad * 2; const ch = k.h - pad * 2;
      ...
      for (let L = 0; L < layers.length - 1; L++) {
        const a = nodes.filter(n => n.L === L); const b = nodes.filter(n => n.L === L + 1);
        for (const x of a) {
    ...
    layout();
    let lw = k.w; let lh = k.h; let last = 0;
    const stop = U.loop((t) => {
      ...
      for (const p of pulses) {
        ...
        p.t += 0.04;
        const a = p.node; const b = p.target;
