(function () {
  ...
  window.HPX['word-cascade'] = function (el) {
    ...
    const WORDS = ['AI', '知识', 'Graph', 'Claude', 'LLM', 'Agent', 'Vector', 'RAG', 'Token', '神经', 'Prompt', 'Chain', 'Skill', 'Code', 'Cloud', 'GPU', 'Flow', '推理', 'Data', 'Model'];
    let items = [];
    let last = -1;
    const piles = {}; // column -> stack height
    const stop = U.loop((t) => {
      ...
      for (const it of items) {
        if (!it.landed) {
          ...
          if (it.y >= floor) {
            ...
            if ((piles[col] || 0) > k.h * 0.8)
              piles[col] = 0; // reset if too high
          }
        }
        ctx.fillStyle = it.c;
