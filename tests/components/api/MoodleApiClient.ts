...
export class MoodleApiClient {
    ...
    @atc('MAC-6', { story: 'UNC-RE-1', feature: 'Enrolment' })
    async enrolUser(courseId: number, userId: number, roleId: number = 5): Promise<void> {
        await this.call<Record<string, unknown>>('enrol_manual_enrol_users', {
    ...
    async getGradeItems(courseId: number): Promise<GradeItem[] | null> {
        try {
            ...
            if (!result || !result.gradeItems || result.gradeItems.length === 0)
                return null;
