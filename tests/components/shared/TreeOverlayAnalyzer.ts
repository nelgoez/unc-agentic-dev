  ...
  static compare(
    ...
    if (studentApiAvailable) {
      ...
      for (const [cmid, adminNode] of adminGraph.nodes) {
        if (cmid === 0) continue
        if (!studentApiGraph.nodes.has(cmid)) continue
        if (studentUiGraph.nodes.has(cmid)) continue

        const isConditionReferenced = options?.conditionReferencedCmids
          ? options.conditionReferencedCmids.has(cmid)
          : true
        if (!isConditionReferenced) continue
    ...
    for (const mismatched of adminVsStudentUi.mismatchedNodes) {
      if (mismatched.field !== 'hasViewLink') continue
      if (!mismatched.expected && mismatched.actual) continue
...
function detectDuplicates(
  ...
  for (let i = 0; i < nodes.length; i++) {
    for (let j = i + 1; j < nodes.length; j++) {
      const a = nodes[i]
      const b = nodes[j]
      if (a.sectionNumber !== b.sectionNumber) continue
      ...
      if (genericFiles.includes(filenameA) || genericFiles.includes(filenameB)) continue

      // Both name AND file must be similar for duplicates with uploaded files
      if (bothHaveFiles && fileSim < 0.5) continue

      // Name similarity must be >= 0.7, or file similarity >= 0.8
      if (sim < 0.7 && fileSim < 0.8) continue
      ...
      if (meaningfulWords < 3 && !bothHaveFiles) continue
      if (meaningfulWords < 2 && bothHaveFiles) continue
